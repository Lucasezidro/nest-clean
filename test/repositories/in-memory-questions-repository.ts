...
import { InMemoryAttachmentsRepository } from './in-memory-upload-attachment'
...
export class InMemoryQuestionsRepository implements QuestionRepository {
  ...
  constructor(
    ...
    private attahcmentsRepository: InMemoryAttachmentsRepository,
  ...
  async findByDetailsBySlug(slug: string) {
    ...
    const questionAttachments = this.questionAttachmentsRepository.items.filter(
      (questionAttahcment) => {
        return questionAttahcment.questionId.equals(question.id)
      },
    )

    const attachments = questionAttachments.map((questionAttachment) => {
      const attachment = this.attahcmentsRepository.items.find((attachment) => {
